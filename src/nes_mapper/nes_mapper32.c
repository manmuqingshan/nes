...
static void mapper32_update_prg(nes_t* nes) {
    ...
    if (r->prg_mode == 0) {
        ...
        nes_load_prgrom_8k(nes, 1, r->prg[1]);
        nes_load_prgrom_8k(nes, 2, r->prg[0]);
        nes_load_prgrom_8k(nes, 3, last);
    }
}
...
static void nes_mapper_write(nes_t* nes, uint16_t address, uint8_t data) {
    ...
    case 0xA000u:
        ...
        mapper32_update_prg(nes);
        break;
    case 0xB000u: /* CHR slots 0-3 */
    case 0xC000u: /* CHR slots 4-7 */
        {
            uint8_t slot = (uint8_t)(((address & 0xF000u) == 0xB000u ? 0u : 4u) + (address & 0x03u));
            r->chr[slot] = data;
            if (nes->nes_rom.chr_rom_size > 0)
