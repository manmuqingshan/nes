...
typedef struct {
    ...
    uint8_t reg5100;    /* Security byte (last value written to $5100) */
} nes_mapper163_t;

static void mapper163_update_prg(nes_t* nes) {
 ...
 * $5100: security unlock byte (accept all writes)
 * $5000: bits[5:0] = 32KB PRG bank select
 * $5200: extended bank bits (ignored for MCU target)
 * $5300: no-op
 */
static void nes_mapper_apu(nes_t* nes, uint16_t address, uint8_t data) {
    ...
    case 0x5000:
        ...
        mapper163_update_prg(nes);
        break;
    /* $5200 and $5300 intentionally ignored for MCU efficiency */
    default:
        break;
    }
}
...
/*
 * Reads from $4020-$5FFF:
 * $5100-$5103: return bitwise inverse of the last security byte written to $5100.
 */
static uint8_t nes_mapper_read_apu(nes_t* nes, uint16_t address) {
    nes_mapper163_t* m = (nes_mapper163_t*)nes->nes_mapper.mapper_register;
    if (address >= 0x5100 && address <= 0x5103) {
        return (uint8_t)(~m->reg5100);
    }
    return 0;
}
